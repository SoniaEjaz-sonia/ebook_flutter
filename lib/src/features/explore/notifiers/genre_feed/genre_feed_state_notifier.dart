    ...
    state.maybeWhen(
      loadSuccess: (books, loadingMore) async {
        if (mounted) {
          ...
          final failureOrSuccess = await _exploreRepository.getGenreFeed(url + '&page=$page');
          failureOrSuccess.fold(
            (failure) {
              if (mounted) state = const GenreFeedState.loadFailure();
            },
            (success) {
              if (mounted) {
                List<Entry> newItems = List.from(books)..addAll(success.feed?.entry ?? []);
                state = GenreFeedState.loadSuccess(
                  loadingMore: false,
                  books: newItems,
                );
...
final genreFeedStateNotifierProvider =
    StateNotifierProvider.family.autoDispose<GenreFeedStateNotifier, GenreFeedState, String>(
  (ref, link) {
