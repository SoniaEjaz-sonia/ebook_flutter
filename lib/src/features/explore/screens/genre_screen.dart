...
import '../../common/widgets/book_list_item.dart';
    ...
    if (_scrollController.position.pixels == _scrollController.position.maxScrollExtent) {
      ref.watch(genreFeedStateNotifierProvider(widget.url)).maybeWhen(
        loadSuccess: (_, loadingMore) {
          if (!loadingMore) {
            page += 1;
            ref.read(genreFeedStateNotifierProvider(widget.url).notifier).paginate(page);
              ...
              loadSuccess: (books, loadingMore) {
                return ListView(
                  ...
                  children: <Widget>[
                    ...
                    if (loadingMore) const SizedBox(height: 80.0, child: LoadingWidget()),
                  ],
                );
              },
            ),
